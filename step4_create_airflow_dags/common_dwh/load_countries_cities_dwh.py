...
def loadCountriesDWH(isUpdate=False):
    ...
    df_cnt = spark.sql("select * from select * from tmp.countries") # temp table Countries
    df_reg_dwh = spark.sql("select * from whdb.regions") # DWH table list of Regions
    df_cont_dwh = spark.sql("select * from whdb.continents") # DWH table list of Continents
    df_cont_stg = spark.sql("select * from staging.country_continents") # staging table Country Codes + Continents
    df_cpt_stg = spark.sql("select * from staging.country_capital") # staging table Country Codes + Capitals
    df_names_stg = spark.sql("select * from staging.country_names") # staging table ISO2 country codes + country names
    
    # -- if create: start --
    if not isUpdate:
        df_mis_cnt = spark.sql("select * from tmp.missing_coutries") # temp table Missing Country Names
        df_hist_cnt = spark.sql("select * from tmp.missing_hist_coutries") # temp table History Countries
    ...
    if isUpdate:
        ...
        df = spark.sql("select * from select * from whdb.countries")
        ...
        df.write.format("orc") \
          .partitionBy("upd_date")
          .mode("overwrite") \
          .saveAsTable("whdb.countries_uptodate")
    # -- if update: end
...
def loadCountryCodesDWH():
    ...
    df_names_stg = spark.sql("select * from staging.country_names") # staging table ISO2 country codes + country names
    df_iso3_stg = spark.sql("select * from staging.country_iso3") # staging table ISO3 + ISO2 country codes
    df_curr_stg = spark.sql("select * from staging.country_currencies") # staging table Currency Codes
    df_curr_dwh = spark.sql("select * from whdb.currencies") # DWH table Currency Codes
    df_phones_stg = spark.sql("select * from staging.country_phones") # staging table Phone Codes
    ...
    df.write.format("orc") \
      .mode("overwrite") \
      .saveAsTable("whdb.country_codes")
...
def loadCodesInCntDWH(isUpdate=False):
    ...
    df_cnt = spark.sql("select * from whdb."+table_name).drop("upd_date") # DWH table Countries / up-to-date Countries
    
    df_cc_dwh = spark.sql("select * from whdb.country_codes") # DWH table Country Codes
    ...
    df = df_cc_dwh.join(df_cnt.where("parent_country_id is null"), join_expr, "outer") \
                  .withColumn("iso2_code_tmp", f.col("iso2_code")) \
    ...
    df_tmp.write.format("orc") \
          .mode("overwrite") \
          .saveAsTable("tmp.codes_in_countries")
    ...
    df.write.format("orc") \
      .mode("overwrite") \
      .saveAsTable("whdb.codes_in_countries")
...
def loadCitiesDWH(isUpdate=False):
    ...
    df_cities_tmp = spark.sql("select * from tmp.cities") # temp table ISO2 country codes + country names
    df_cc_tmp = spark.sql("select * from tmp.codes_in_countries") # temp table Codes in Countries
    ...
    df = df.withColumn("CountryUpd", f.col("Country"))
    ...
    if isUpdate:
        ...
        # this dataset will be used in other data transformation functions
        df_tmp = spark.sql("select * from tmp.cities2")
        ...
        df_tmp.write.format("orc") \
              .partitionBy("upd_date")
              .mode("overwrite") \
              .saveAsTable("tmp.cities2_uptodate")
        ...
        df_tmp.write.format("orc") \
              ...
              .mode("append") \
              .saveAsTable("whdb.cities_uptodate")
    ...
    if not isUpdate:
        ...
        df_mis_cities = spark.sql("select * from tmp.missing_cities") # temp table with mismatches in City/Nobel Laureates
        ...
        df_mis_cities.write.format("orc") \
                     .mode("overwrite") \
                     .saveAsTable("tmp.missing_cities")
    # -- if create: end --
