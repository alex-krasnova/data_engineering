...
def loadLaureatesDWH(isUpdate=False):
    spark_start()
    
    df_nobel_tmp = spark.sql("select * from tmp.nobel_laureates") # temp table Nobel Laureates with updated columns
    df_gend_dwh = spark.sql("select * from whdb.genders") # DWH table Genders
    ...
    df_cnt_dwh = spark.sql("select * from whdb."+table_name).drop("upd_date") # DWH table Countries / up-to-date Countries
    ...
    df_cities_tmp = spark.sql("select * from tmp."+table_name).drop("upd_date") # temp table Cities / up-to-date Countries
    
    if not isUpdate:
        df_mis_cities = spark.sql("select * from tmp.missing_cities") # temp table Missing Cities
    ...
    if isUpdate:
        ...
        df_prev = spark.sql("select * from whdb.societies") # previous records
    ...
    if not isUpdate:
        ...
        # join missing city names to the Laureates dataframe - use the table tmp.missing_cities created before
    ...
    if isUpdate:
        ...
        df_prev = spark.sql("select * from whdb.laureate_persons") # previous records
        df = df_prev.union(df) # previous + current records
...
def loadOrganizationsDWH(isUpdate=False):
    ...
    if not isUpdate:
        df_mis_orgs = spark.sql("select * from tmp.missing_orgs") # temp table missing parents organization
        df_mis_cities = spark.sql("select * from tmp.missing_cities") # temp table Missing Cities
    ...
    if isUpdate:
        ...
        df_prev = spark.sql("select * from tmp.organizations") # previous records
        df = df_prev.union(df) # previous + current records
    ...
    df.write.format("orc") \
      .partitionBy("upd_date")
      .mode("overwrite") \
      .saveAsTable("whdb.organizations")
...
def loadPersonsInOrgsDWH(isUpdate=False):
    spark_start()
    
    df_nob_tmp = spark.sql("select * from tmp.nobel_laureates").drop("upd_date") # temp table Nobel Laureates with updated columns
    df_org_tmp = spark.sql("select * from tmp.organizations").drop("upd_date") # temp table Organizations
    ...
    if isUpdate:
        ...
        df_prev = spark.sql("select * from whdb.persons_in_orgs") # previous records
        df = df_prev.union(df) # previous + current records
    ...
    df.write.format("orc") \
      .mode("overwrite") \
      .saveAsTable("whdb.persons_in_orgs")
