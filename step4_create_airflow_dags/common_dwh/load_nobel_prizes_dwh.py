...
def loadNobelPrizesDWH():
    spark_start()
    
    df_nob_tmp = spark.sql("select * from tmp.nobel_laureates") # temp table Nobel Laureates with updated columns
    df_cat_dwh = spark.sql("select * from whdb.categories") # DWH table Categories
    df_pt_dwh = spark.sql("select * from whdb.prize_types").drop("upd_date") # DWH table Prize Types
    df_lt_dwh = spark.sql("select * from whdb.laureate_types") # DWH table Laureate Types
