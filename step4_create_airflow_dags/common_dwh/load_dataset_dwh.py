...
def loadDatasetDWH(isUpdate=False):
    spark_start()
    spark.conf.set("spark.sql.adaptive.enabled",True)
    spark.conf.set("spark.sql.adaptive.skewJoin.enabled",True)
    spark.conf.set("spark.sql.adaptive.coalescePartitions.enabled",True)
    
    df_nob_tmp = spark.sql("select * from tmp.nobel_laureates") # temp table Nobel Laureates with updated columns
    df_laur_dwh = spark.sql("select * from whdb.laureate_persons").drop("upd_date") # DWH table Laureates Persons
    df_org_dwh = spark.sql("select * from whdb.organizations").drop("upd_date") # DWH table Organizations
    df_cont_dwh = spark.sql("select * from whdb.continents") # DWH table Continents
    df_reg_dwh = spark.sql("select * from whdb.regions") # DWH table Regions
    ...
    df_city_dwh = spark.sql("select * from whdb."+table_name).drop("upd_date") # DWH table Countries / up-to-date Cities
    ...
    df.write.format("orc") \
      .mode("append") \
      .saveAsTable("tmp.dataset")
    
    # Organization columns
    
    # take all dataset records, not delta only
    df = spark.sql("select * from tmp.dataset")
