...
def loadFromNobelDfDWH(isUpdate=False):
    spark_start()
    
    df = spark.sql("select * from tmp.nobel_laureates") # temp table Nobel Laureates with updated columns
    ...
    if isUpdate:
        ...
        df_prev = spark.sql("select * from whdb.prize_types") # previous records
    ...
    if not isUpdate:
        ...
        df_tmp.write.format("orc") \
              .mode("overwrite") \
              .saveAsTable("whdb.laureate_types")
...
def loadRegionsDWH():
    ...
    df = spark.sql("select * from select * from tmp.countries") # temp table Countries
    ...
    df.write.format("orc") \
      .mode("overwrite") \
      .saveAsTable("whdb.regions")
    
    spark_stop()

# create table Continents in Data Warehouse
def loadContinentsDWH():
    spark_start()
    
    df = spark.sql("select * from staging.country_continents")
    ...
    df.write.format("orc") \
      .mode("overwrite") \
      .saveAsTable("whdb.continents")
    
    spark_stop()

# create table Currencies in Data Warehouse
def loadCurrencyCodesDWH():
    spark_start()
    
    df = spark.sql("select * from staging.country_currencies") # staging table Currency Codes
