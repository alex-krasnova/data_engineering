...
def createTmpCountriesTbl(isUpdate=False):
    ...
    df_cnt = spark.sql("select * from tmp.countries_delta") # staging table (delta) Countries
    ...
    df_cnt.write.format("orc") \
          .mode("overwrite") \
          .saveAsTable("tmp.countries")
    
    # -- if create: start
    if not isUpdate:
    
        df_names = spark.sql("select * from staging.country_names") # staging table ISO2 Codes + Country Names
...
drop table if exists tmp.missing_hist_coutries purge;
create table tmp.missing_hist_coutries (
    country_name  varchar(50)
) stored as orc;
insert into table tmp.missing_hist_coutries
  values 
...
def createTmpLaureatesTbl():
    ...
    df = spark.sql("select * from tmp.nobel_laureates_delta") # staging table (delta) Nobel Laureates
    ...
    df.write.format("orc") \
      .mode("overwrite") \
      .saveAsTable("tmp.nobel_laureates")
...
def createTmpCitiesTbl():
    ...
    df = spark.sql("select * from tmp.cities_delta") # staging table (delta) Cities
...
/usr/bin/beeline -u jdbc:hive2://*** -n *** -p *** <<END_SQL
drop table if exists tmp.missing_cities purge;
create table tmp.missing_cities (
    mis_city_name varchar(50), 
    orig_city_name varchar(50),
    orig_region varchar(5),
    orig_country varchar(2)
) stored as orc;
insert into table tmp.missing_cities
  values 
...
/usr/bin/beeline -u jdbc:hive2://*** -n *** -p *** <<END_SQL
drop table if exists tmp.missing_orgs purge;
create table tmp.missing_orgs (
    org_name  varchar(50),
    org_country  varchar(50),
    org_city  varchar(50)
) stored as orc;
insert into table tmp.missing_orgs
  values 
